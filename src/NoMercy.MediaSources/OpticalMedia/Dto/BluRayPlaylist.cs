...
public partial class BluRayPlaylist
{
    ...
    public static BluRayPlaylist Parse(string input)
    {
        ...
        foreach (string line in lines)
        {
            ...
            if (line.StartsWith("Complete name"))
            {
                ...
                playlist.PlaylistId = (
                    value.Split('\\').LastOrDefault()?.Split('.').FirstOrDefault()
                ).OrEmpty();
            }
            ...
            {
                currentVideo = new() { StreamIndex = index++ };
            }
            else if (line.StartsWith("Audio"))
            {
                currentAudio = new() { StreamIndex = index++ };
            }
            else if (line.StartsWith("Text"))
            {
                currentSubtitle = new() { StreamIndex = index++ };
            }
            ...
            if (currentVideo != null)
            {
                ...
                {
                    currentVideo.FrameRate = double.Parse(
                        Regex.Match(line, "[0-9.]+").Value,
    ...
    private static TimeSpan ParseDuration(string input)
    {
        Match match = Regex.Match(
            input,
            @"(?:(\d+)\s*hour[s]?)?\s*(?:(\d+)\s*min[s]?)?\s*(?:(\d+)\s*s)"
        );
