        ...
        [
            Policy.HandleResult<HttpResponseMessage>(r => !r.IsSuccessStatusCode)
                ...
                .WaitAndRetryAsync(retryCount, _ => TimeSpan.FromSeconds(2),
                    ...
                    {
                        Console.WriteLine(
                            $"Retry {rtCount}: {result?.Result?.RequestMessage?.RequestUri?.ToString() ?? baseUrl ?? "unknown"} after {timeSpan.TotalSeconds} seconds due to: {result.Exception?.Message ?? result.Result?.StatusCode.ToString()}");
                    }),
