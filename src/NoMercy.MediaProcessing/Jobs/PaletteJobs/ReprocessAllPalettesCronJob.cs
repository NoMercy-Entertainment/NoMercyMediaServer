...
public class ReprocessAllPalettesCronJob : ICronJobExecutor
{
    ...
    public async Task ExecuteAsync(string parameters, CancellationToken cancellationToken = default)
    {
        ...
        else
        {
            _logger.LogInformation(
                "Reprocess all palettes: processed {Count} items this tick",
                totalProcessed
            );
        }
    }
    ...
    {
        List<Season> items = _context
            .Seasons.Where(x => string.IsNullOrEmpty(x._colorPalette))
        ...
        await Parallel.ForEachAsync(
            ...
            async (item, _) =>
            {
                try
                {
                    item._colorPalette = await MovieDbImageManager.ColorPalette(
                        "poster",
                        item.Poster
                    );
                }
    ...
    {
        List<Collection> items = _context
    ...
    {
        List<Recommendation> items = _context
    ...
    private async Task<int> ProcessAlbums(CancellationToken ct)
    {
        ...
        await Parallel.ForEachAsync(
            ...
            async (item, _) =>
            {
                try
                {
                    item._colorPalette = await CoverArtImageManagerManager.ColorPalette(
                        "cover",
                        new(AppFiles.MusicImagesPath + item.Cover)
                    );
                }
                catch (Exception)
                {
