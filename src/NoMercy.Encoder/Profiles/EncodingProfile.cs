...
)
{
    ...
    // Default to PreferHardware so a preset hits NVENC/AMF/QSV/VideoToolbox when
    // one is present. Presets that need CPU quality (the archival ones) opt into
    // PreferQuality explicitly.
    ...
    public HdrPolicies HdrPolicies { get; init; } = HdrPolicies.PassthroughWhenPossible;
    public HdrOptions? HdrOptions { get; set; }
