...
public partial class VideoAudioFile(MediaAnalysis fMediaAnalysis, string ffmpegPath) : Classes
{
    ...
    public string GetFullCommand()
    {
        ...
        if (Container.IsVideo)
        {
            ...
            foreach (GpuAccelerator accelerator in Accelerators) command.Append($" {accelerator.FfmpegArgs} ");
        }
        ...
        if (Container.IsVideo && Accelerators.Count > 0) command.Append(" -gpu any ");
        ...
        foreach (BaseVideo stream in Container.VideoStreams)
        {
            ...
            if (stream.Scale.H == 0)
                stream.Scale = new()
                {
                    ...
                    H = stream.Scale.W * (fMediaAnalysis.PrimaryVideoStream!.Height /
                                          fMediaAnalysis.PrimaryVideoStream!.Width)
                };
        ...
        foreach (BaseSubtitle stream in Container.SubtitleStreams)
        {
            ...
            if (stream.ConvertSubtitle) ConvertSubtitle = true;
