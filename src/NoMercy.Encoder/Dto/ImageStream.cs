...
public class ImageStream
{
    ...
    public string DisplayAspectRatio => NumberConverter.NormalizeAspectRatio(Width, Height);
    ...
    public ImageStream(FfprobeSourceDataStream ffprobeSourceDataStream)
    {
        ...
        Filename = ffprobeSourceDataStream.Tags.TryGetValue("filename", out string? filename)
            ? filename
            : null;

        Title = ffprobeSourceDataStream.Tags.TryGetValue("title", out string? title) ? title : null;
        Mimetype = ffprobeSourceDataStream.Tags.TryGetValue("mimetype", out string? mimeType)
            ? mimeType
            : "und";
    }
}
