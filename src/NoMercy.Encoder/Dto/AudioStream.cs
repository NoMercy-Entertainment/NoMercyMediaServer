...
public class AudioStream
{
    ...
    public string? LanguageName => IsoLanguageMapper.GetLanguageName(Language.OrEmpty());
    ...
    [JsonProperty("is_visual_impaired")]
    ...
    public AudioStream(FfprobeSourceDataStream ffprobeSourceDataStream)
    {
        ...
        Language = ffprobeSourceDataStream.Tags.TryGetValue("language", out string? language)
            ? language
            : "und";
        Duration = ffprobeSourceDataStream.Tags.TryGetValue(
            $"DURATION-{Language}",
            out string? duration
        )
            ? duration.ToSeconds()
            : ffprobeSourceDataStream.Duration.ToSeconds();
        Size = ffprobeSourceDataStream.Tags.TryGetValue(
            $"NUMBER_OF_BYTES-{Language}",
            out string? numberOfBytes
        )
            ? numberOfBytes.ToLong()
            : 0;
        ...
        IsDefault =
            ffprobeSourceDataStream.Disposition.TryGetValue("default", out int defaultValue)
            && defaultValue == 1;
        IsDub =
            ffprobeSourceDataStream.Disposition.TryGetValue("dub", out int dubValue)
            && dubValue == 1;
        IsForced =
            ffprobeSourceDataStream.Disposition.TryGetValue("forced", out int forcedValue)
            && forcedValue == 1;
        IsVisualImpaired =
            ffprobeSourceDataStream.Disposition.TryGetValue(
                "visual_impaired",
                out int visualImpairedValue
            )
            && visualImpairedValue == 1;
