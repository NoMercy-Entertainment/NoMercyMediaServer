...
public static class Register
{
    ...
    private static async Task AssignServer()
    {
        ...
        authClient.SetDefaultHeaders(Config.UserAgent, Globals.Globals.AccessToken);
        string response =
            await authClient.SendAndReadAsync(HttpMethod.Post, "assign", new FormUrlEncodedContent(serverData));

        ServerRegisterResponseData? data = response.FromJson<ServerRegisterResponse>()?.Data;

        if (data is null || data.Status == "error") throw new("Failed to assign Server");
...
#pragma warning restore CS0618 // Type or member is obsolete
    ...
    public static async Task GetTunnelAvailability()
    {
        ...
        authClient.SetDefaultHeaders(Config.UserAgent, Globals.Globals.AccessToken);
        string response =
            await authClient.SendAndReadAsync(HttpMethod.Post, "tunnel", new FormUrlEncodedContent(serverData));

        ServerTunnelAvailabilityResponse? data =
            JsonConvert.DeserializeObject<ServerTunnelAvailabilityResponse>(response);
