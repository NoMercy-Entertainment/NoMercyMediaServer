...
public record LibraryResponseDto
{
    ...
    [JsonProperty("data")]
    public List<LibraryResponseItemDto> Data { get; set; } = [];

    public static async Task<List<Movie>> GetLibraryMovies(
    ...
    {
        IIncludableQueryable<Movie, Certification> query = mediaContext
            .Movies.AsNoTracking()
            .Where(movie => movie.Library.Id == libraryId)
            .Where(movie => movie.Library.LibraryUsers.Any(u => u.UserId.Equals(userId)))
            .Where(libraryMovie =>
                libraryMovie.VideoFiles.Any(videoFile => videoFile.Folder != null) == true
            )
    ...
    public static async Task<List<Tv>> GetLibraryShows(
        MediaContext mediaContext,
        Guid userId,
        Ulid libraryId,
        string language,
    ...
    {
        IIncludableQueryable<Tv, Certification> query = mediaContext
            .Tvs.AsNoTracking()
            .Where(tv => tv.Library.Id == libraryId)
            .Where(tv => tv.Library.LibraryUsers.Any(u => u.UserId.Equals(userId)))
            .Where(libraryTv =>
                libraryTv.Episodes.Any(episode =>
                    episode.VideoFiles.Any(videoFile => videoFile.Folder != null) == true
                ) == true
            )
            .Include(tv =>
                tv.Episodes.Where(episode =>
                    episode.SeasonNumber > 0 && episode.VideoFiles.Count != 0
                )
            )
    ...
    public static readonly Func<
        ...
        int,
        Task<Library>
    > GetLibrary = EF.CompileAsyncQuery(
        (MediaContext mediaContext, Guid userId, Ulid id, string language, int take, int page) =>
            mediaContext
                ...
                .Skip(page * take)
                .Include(library =>
                    library.LibraryMovies.Where(libraryMovie =>
                        libraryMovie.Movie.VideoFiles.Any(videoFile => videoFile.Folder != null)
                        == true
                    )
                )
                ...
                .Include(library => library.LibraryMovies)
                    .ThenInclude(libraryMovie =>
                        libraryMovie.Movie.Media.Where(media =>
                            media.Iso6391 == language || media.Iso6391 == "en"
                        )
                    )
                .Include(library => library.LibraryMovies)
                    .ThenInclude(libraryMovie =>
                        libraryMovie.Movie.Images.Where(image =>
                            image.Iso6391 == language || image.Iso6391 == "en"
                        )
                    )
                .Include(library => library.LibraryMovies)
                ...
                .Include(library => library.LibraryMovies)
                    .ThenInclude(libraryMovie =>
                        libraryMovie.Movie.Translations.Where(translation =>
                            translation.Iso6391 == language || translation.Iso6391 == "en"
                        )
                    )
                .Include(library => library.LibraryMovies)
                    .ThenInclude(libraryMovie => libraryMovie.Movie.CertificationMovies)
                        .ThenInclude(certificationMovie => certificationMovie.Certification)
                .Include(library =>
                    library.LibraryTvs.Where(libraryTv =>
                        libraryTv.Tv.Episodes.Any(episode =>
                            episode.VideoFiles.Any(videoFile => videoFile.Folder != null) == true
                        ) == true
                    )
                )
                ...
                .Include(library => library.LibraryTvs)
                    .ThenInclude(libraryTv =>
                        libraryTv.Tv.Media.Where(media =>
                            media.Iso6391 == language || media.Iso6391 == "en"
                        )
                    )
                .Include(library => library.LibraryTvs)
                    .ThenInclude(libraryTv =>
                        libraryTv.Tv.Images.Where(image =>
                            image.Iso6391 == language || image.Iso6391 == "en"
                        )
                    )
                .Include(library => library.LibraryTvs)
                ...
                .Include(library => library.LibraryTvs)
                    .ThenInclude(libraryTv =>
                        libraryTv.Tv.Translations.Where(translation =>
                            translation.Iso6391 == language || translation.Iso6391 == "en"
                        )
                    )
                .Include(library => library.LibraryTvs)
