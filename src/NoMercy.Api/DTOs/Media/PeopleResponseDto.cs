...
public record PeopleResponseDto
{
    ...
    public static async Task<List<PeopleResponseItemDto>> GetPeople(
    ...
    {
        IIncludableQueryable<Person, IEnumerable<Translation>> query = mediaContext
            .People.AsNoTracking()
            .Where(person =>
        ...
        return people.Select(person => new PeopleResponseItemDto(person)).ToList();
    }
}
