...
**Build:** 182 commits ahead of `master` · 0 errors · 0 warnings · 822 encoder tests + 6 repository tests passing (+136 added this session; +15 live transcode HTTP surface).
...
| 5. Live Transcode Strategy | ⚠️ HTTP surface landed | `LiveEncoder.StartAsync` still stubs the FFmpeg spawn, but server-side HTTP wiring is in: `ILiveStreamingService` (singleton runtime map with background segment drainer + indexed buffer), `LivePlaylistBuilder` (pure HLS `.m3u8` emitter — EVENT / VOD + ENDLIST + InvariantCulture `EXTINF`), `LiveTranscodeController` exposes POST/GET/DELETE under `/api/v1/streaming/live/sessions/...`. 15 new tests. Pending: actual FFmpeg spawn in `LiveEncoder`. |
...
6. ⚠️ **Phase 5 — Live transcode.** HTTP surface landed: `ILiveStreamingService` + `LivePlaylistBuilder` + `LiveTranscodeController` (POST /sessions, GET /playlist.m3u8, GET /segment/{i}.ts with Accept-Ranges, POST /position, DELETE). Auth: resolves `VideoFile` by id + verifies `Library.LibraryUsers` membership (never accepts raw paths). Remaining: wire FFmpeg spawn inside `LiveEncoder.StartAsync` so the segment channel actually produces real segments.
