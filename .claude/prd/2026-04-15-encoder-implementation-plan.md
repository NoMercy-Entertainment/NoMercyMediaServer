...
| 5. Live Transcode Strategy | ✅ MVP end-to-end | `LiveFfmpegRunner` spawns the real FFmpeg HLS muxer (`-f hls -hls_time N -hls_list_size 0 -hls_playlist_type event -hls_segment_filename seg_%05d.ts -progress pipe:1`); a 500ms poller parses `index.m3u8` → `Segment` records pushed into the session channel; `-progress pipe:1` feeds `ProgressParser` → `session.SetSpeed`. `LiveSession.RunnerCancellation` fires on `DisposeAsync` so a client-driven `DELETE /sessions/{id}` kills FFmpeg. `LiveEncoder` owns registration with `ILiveStreamingService` including the per-session scratch dir; `RemoveAsync` deletes it best-effort. HTTP surface, drainer buffer, playlist builder, runner, scratch cleanup — 37 new tests. Still pending: in-session quality change + seek re-spawn (clients tear down + start a new session today). |
| 6. Distribution | ⚠️ local done | `Distribution/` has `IWorkerDispatcher` + `LocalWorkerDispatcher` + `EncodeTask` (QualityVariant / TimeChunk). Sequential task dispatch via `IFfmpegExecutor` with per-task failure isolation. `IHardwareBenchmark` + `JsonSpeedIndexStore` + `HardwareBenchmarkHostedService` calibrate on startup (30-day staleness window), dashboard endpoint (`GET /dashboard/hardware/benchmark` + owner-only `POST /run`). Remote workers still require network protocol — Tier 4. |
...
| 11. DRM & Encryption | ⚠️ AES-128 HLS done | `IDrmProcessor` + `Aes128HlsDrmProcessor` ship. `DrmConfig` on `EncodingProfile` → `OutputPlan.Drm` → `BuildStage.ApplyDrmPreparationAsync` writes key + keyinfo to the output directory and splices `-hls_key_info_file` into each video output's extra flags. ffmpeg handles AES-128-CBC segment encryption + EXT-X-KEY tags inline. CENC for DASH (Widevine / PlayReady / FairPlay) still deferred — needs `shaka-packager`. |
| 12. Presets & Automation | ✅ all shipped | `INotificationDispatcher` + `WebhookNotificationDispatcher` (POST with exponential-backoff retries) + `EncodingNotificationSubscriber` hosted service wiring the event bus; configurable via `EncoderOptions.NotificationWebhookUrls`. Watch-folder auto-encode shipped earlier as `AutoEncodeSubscriber`. Preset library: `EncodingPreset` model + EF migration + `EncodingPresetRepository` + `EncodingPresetsController` (CRUD + `/import` + `/export`) + `IPresetResolver` with cycle-safe parent-chain inheritance + 5 built-in presets seeded on boot. |
...
6. ✅ **Phase 5 — Live transcode.** MVP complete. `LiveFfmpegRunner` spawns FFmpeg HLS event playlist, 500ms poller parses `index.m3u8` and pushes `Segment` records into the session channel, `-progress pipe:1` drives `session.SetSpeed`. Client DELETE → session DisposeAsync cancels the runner CT → FFmpeg killed. End-to-end: `POST /sessions` returns id + playlist URL, client hits `GET playlist.m3u8` and `GET segment/{i}.ts` (range-aware), `POST /position` reports playback. Remaining: on-the-fly quality change / seek require session tear-down + new session today.
