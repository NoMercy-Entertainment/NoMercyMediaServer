...
public class AuthExpirationTests
{
    ...
    public void ExpirationLogic_ValidTokenNotExpired()
    {
        ...
        Assert.False(condition: expired, userMessage: "A token with 5+ days remaining should NOT be marked expired");
    }
    ...
    public void ExpirationLogic_ExpiredTokenIsExpired()
    {
        ...
        int expiresInDays = -2;
        bool expired = expiresInDays < 0;
        Assert.True(condition: expired, userMessage: "A token that expired 2 days ago should be marked expired");
    }
    ...
    public void ExpirationLogic_ZeroDaysIsNotExpired()
    {
        ...
        Assert.False(
            condition: expired,
            userMessage: "A token at exactly the boundary (0 days) should NOT be marked expired"
        );
    }
    ...
    public void ExpirationLogic_NegativeOneDayIsExpired()
    {
        ...
        int expiresInDays = -1;
        bool expired = expiresInDays < 0;
        Assert.True(condition: expired, userMessage: "A token 1 day past the refresh window should be marked expired");
    }
}
