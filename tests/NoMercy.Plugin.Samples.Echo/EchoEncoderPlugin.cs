...
public class EchoEncoderPlugin : IEncoderPlugin
{
    ...
    public Guid Id { get; } = Guid.Parse(input: "11111111-2222-3333-4444-555555555555");
    public Version Version { get; } = new(major: 0, minor: 1, build: 0);

    public void Initialize(IPluginContext context)
    {
        context.Logger.LogInformation(message: "Echo plugin initialized");
    }

    public EncodingProfile GetProfile(MediaInfo info) =>
        new()
        {
            Name = $"Echo-{Path.GetFileName(path: info.FilePath)}",
            VideoCodec = "h264",
            AudioCodec = "aac",
        };
    ...
    public void Dispose()
    {
        string? markerPath = Environment.GetEnvironmentVariable(
            variable: "NOMERCY_TEST_ECHO_DISPOSAL_MARKER"
        );
        if (markerPath is not null)
            File.WriteAllText(path: markerPath, contents: "disposed");
    }
}
