...
public class IStorageFacadeTests
{
	private static Mock<IStorageDriver> NewDriver()
	{
		...
		driver
			...
			.Returns<string, string>((parent, child) =>
			{
	...
	public async Task ReadAsync_rejects_absolute_paths_before_driver_sees_them(
		string absolutePath
	)
	{
		...
		Func<Task> act = async () =>
			await storage.ReadAsync(absolutePath, CancellationToken.None);
	...
	public void Multiple_allowed_roots_path_outside_all_rejected()
	{
		...
		act.Should()
			.Throw<StoragePathNotAllowedException>(
				"path outside all roots must be rejected"
			);
		driver.Verify(
			d => d.FileExists(It.IsAny<string>()),
			Times.Never,
	...
	public void Enforced_guard_rejects_symlink_target_escaping_root()
	{
		...
		act.Should()
			.Throw<StoragePathNotAllowedException>(
				"symlinks escaping the root must be rejected"
			);
	}
	...
	public void CombinePath_trims_redundant_separators()
	{
		...
		combined.Should()
			.Be("parent/child", "redundant separators must be trimmed");
	}
	...
	public void GetParent_returns_parent_directory_segment(string path, string? expectedParent)
	{
		...
		parent.Should()
			.Be(
				expectedParent,
				"GetParent must return the directory or null for root entries"
			);
	}
	...
	public void GetNameWithoutExtension_strips_extension_from_name(
		string path,
		string expectedName
	)
	{
		...
		name.Should()
			.Be(expectedName, "GetNameWithoutExtension must return the last segment without its extension");
	}
	...
	public async Task WriteAsync_creates_parent_directories_automatically()
	{
		...
		driver.Setup(d => d.OpenWrite(It.IsAny<string>(), It.IsAny<bool>()))
			.Returns(sink);
	...
	public async Task OpenWriteAsync_creates_parent_directories_before_opening_stream()
	{
		...
		Stream result = await storage.OpenWriteAsync("dir/subdir/file.mp4", overwrite: true, CancellationToken.None);
		...
		result.Should()
			.BeSameAs(
				stream,
				"the driver's stream must be returned, not wrapped"
			);
	}
	...
	public async Task CopyAsync_creates_parent_directory_for_destination()
	{
		...
		Mock<IStorageDriver> driver = NewDriver();
		driver.Setup(d => d.DirectoryExists(It.IsAny<string>())).Returns(false);
		driver.Setup(d => d.CopyFile(It.IsAny<string>(), It.IsAny<string>(), It.IsAny<bool>()))
			.Callback(() => { }); // no-op
	...
	public void List_returns_scope_relative_paths_not_driver_paths()
	{
		...
		result[0].Path.Should()
			.NotStartWith("/", "scope-relative paths must not start with /");
		result[0].Path.Should()
			.NotStartWith("\\", "scope-relative paths must not start with \\");
		result[0].Path.Should()
			.NotStartWith(
				root,
				"scope-relative paths must not include the root prefix"
			);
	}
	...
	public void List_applies_pattern_filter_via_driver()
	{
		...
		result.Should()
			.ContainSingle("pattern filter must be applied");
		result[0].Path.Should()
	...
	public async Task HashAsync_supports_sha256()
	{
		...
		driver.Setup(d => d.OpenRead(It.IsAny<string>()))
		...
		hash.Should()
			.NotBeNullOrEmpty("hash must be computed");
		hash.Should()
			.Be(
		...
		hash.Length.Should()
			.Be(64, "SHA256 produces 32 bytes = 64 hex chars");
	}
	...
	public async Task HashAsync_supports_md5()
	{
		...
		Mock<IStorageDriver> driver = NewDriver();
		driver.Setup(d => d.OpenRead(It.IsAny<string>()))
			.Returns(new MemoryStream([0x61])); // "a"
		...
		hash.Should()
			.NotBeNullOrEmpty("md5 hash must be computed");
		hash.Length.Should()
			.Be(32, "MD5 produces 16 bytes = 32 hex chars");
	}
	...
	public async Task AcquireLocalPathAsync_returns_absolute_path()
	{
		...
		lease.Path.Should()
			.NotBeNullOrEmpty("lease must contain a path");
		Path.IsPathRooted(lease.Path).Should()
	...
	public async Task WriteAllTextAsync_writes_string_content()
	{
		...
		await storage.WriteAllTextAsync("config.json", "{\"key\": \"value\"}", CancellationToken.None);

		sink.ToArray().Should()
			.Equal(System.Text.Encoding.UTF8.GetBytes("{\"key\": \"value\"}"));
	}
	...
	public async Task MoveDirectoryAsync_validates_both_paths()
	{
		...
		await act.Should()
			.ThrowAsync<StoragePathNotAllowedException>(
				"bad destination path must be rejected"
			);
		driver.Verify(
	...
	public void Reject_device_paths_on_windows()
	{
		...
		act.Should()
			.Throw<StoragePathNotAllowedException>(
				"device paths must be rejected on Windows"
			);
	}
}
