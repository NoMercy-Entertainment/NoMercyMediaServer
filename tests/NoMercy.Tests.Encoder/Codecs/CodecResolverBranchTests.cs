...
public class CodecResolverBranchTests
{
    ...
    [Theory]
    [InlineData(EncoderPreference.ForceSoftware)]
    [InlineData(EncoderPreference.ForceHardware)]
    [InlineData(EncoderPreference.PreferQuality)]
    [InlineData(EncoderPreference.PreferSpeed)]
    [InlineData(EncoderPreference.PreferHardware)]
    public void Copy_codec_returns_copy_encoder_regardless_of_preference(EncoderPreference pref)
    {
        ...
        ResolvedCodec resolved = resolver.Resolve(VideoCodecType.Copy, noCaps, pref);
    ...
    public void Unknown_preference_value_falls_back_to_prefer_hardware()
    {
        ...
        ResolvedCodec resolved = resolver.Resolve(
            VideoCodecType.H264,
            caps,
            (EncoderPreference)99 // bogus enum value
        );
    ...
    public void ForceHardware_GPU_present_but_codec_unsupported_throws()
    {
        ...
        Action act = () =>
            resolver.Resolve(VideoCodecType.Av1, caps, EncoderPreference.ForceHardware);
